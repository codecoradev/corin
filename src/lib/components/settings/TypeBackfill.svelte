<script lang="ts">
   ...
   */
  import { memoryUpdate, system, uteke } from '../../ts/ipc';
  ...
  async function scan() {
    ...
    try {
      const [items, stats] = await Promise.all([
        uteke.list({ limit: 1000 }),
        system.stats().catch(() => null),
      ]);
      // An empty page from a server that reports memories is a failure, not
      // a clean scan — surface it instead of claiming "all typed".
      const total = stats?.total_memories ?? 0;
      if (total > 0 && items.length === 0) {
        scanError = `Server reports ${total} memories but returned none for scanning.`;
        return;
      }
      untyped = items
...
<section class="content-section">
  ...
  <div class="bf-card">
    ...
    {#if scanError}
      <p class="bf-status err">⚠ {scanError}</p>
      <Button variant="secondary" size="sm" onclick={scan}>Retry</Button>
    {:else if scanning}
